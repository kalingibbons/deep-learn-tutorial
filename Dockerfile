...
# and `pip install deep-learn-tutorial` using an artifact store. Faster and more robust.
...
SHELL ["mamba", "run", "-n", "deep-learn-tutorial", "/bin/bash", "-c"]

# Build package
RUN tox -e build
RUN mamba env export -n deep-learn-tutorial -f environment.docker.yml
...
SHELL ["conda", "run", "-n", "deep-learn-tutorial", "/bin/bash", "-c"]
...
# ENTRYPOINT ["conda", "run", "--no-capture-output", "-n", "deep-learn-tutorial", "YOUR_CONSOLE_SCRIPT"]
